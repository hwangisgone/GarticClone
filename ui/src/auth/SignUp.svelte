<script>
	...
	// C++ Call Js
	window.auth_registered = () => { $UIstate = 0; }
	window.auth_usernameUsed = () => { usernameUsed = true; }
	...
	<div class="fixed">
		<button class="btn" on:click={window.auth_registered}>registered</button>
		<button class="btn" on:click={window.auth_usernameUsed}>useused</button>
	</div>

<div class="md:flex w-full h-full ">
	<div class="hidden md:block w-1/2 bg-neutral py-10 px-10 rounded-l-3xl">
	...
	<div class="w-full md:w-1/2 px-5 md:px-10">
		<h1 class="text-4xl lg:text-5xl text-neutral pt-24 font-semibold">Register {$UIstate}</h1>
		...
		<div class="m-7 space-y-2 md:space-y-4">
			<InputContainer labelText="Username">
				...
				class="w-full input input-primary lg:input-lg pl-8 text-neutral-content" />	
